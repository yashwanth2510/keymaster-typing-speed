...
import { Shield, Flame, RotateCcw, Award, Zap, Heart, Orbit, Bomb, Timer } from 'lucide-react';
              ...
              return (
                ...
                >
                  <div className={`drop-shadow opacity-90 ${m.type === 'bomb' ? 'text-rose-500' : m.type === 'shield' ? 'text-emerald-600' : m.type === 'slow' ? 'text-sky-600' : 'text-[#DA6A45]'}`}>
                    {m.type === 'bomb' ? <Bomb className="w-5 h-5" /> : m.type === 'shield' ? <Shield className="w-5 h-5" /> : m.type === 'slow' ? <Timer className="w-5 h-5" /> : <Orbit className="w-5 h-5" />}
                  </div>
